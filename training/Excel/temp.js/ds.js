...
class SparseMatrix {
    ...
    createCell(row, col, rowValue, colValue, value) {
        ...
        console.log("create node for ",value)
        ...
        if (!this.rowHeaders[row]) {
            ...
            console.log("created undefined for row ",value)
            ...
            newNode.prevCol = this.rowHeaders[row];
        } else {
            let current = this.rowHeaders[row];
            let prev = null;
        ...
        if (!this.colHeaders[col]) {
            ...
            console.log("create undefined for col",value);
    ...
    setCell(row, col, value) {
        ...
        if (row == 1){
            this.printMatrixbyrow()
        }
    }

    // Print the matrix
    printMatrixbyrow() {
        for (let row in this.sharedRows) {
            let current = this.rowHeaders[row];
            let rowValues = [];
            while (current) {
    ...
    printMatrixbycol() {
        for (let col in this.sharedCols) {
            let current = this.colHeaders[col];
            let rowValues = [];
            while (current) {
                rowValues.push(`${current.value}`);
